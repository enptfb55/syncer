package server;

import java.awt.FlowLayout;
import java.awt.GridLayout;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.io.File;

import javax.swing.JButton;
import javax.swing.JFrame;
import javax.swing.JLabel;
import javax.swing.JPanel;
import javax.swing.JTextField;
import javax.swing.border.TitledBorder;

import utils.FileUtils;

/**
 * @author samato
 * 
 */
@SuppressWarnings("serial")
public class ServerSync extends JFrame {
	static int port = 2525;
	private static File Dir = new File(
			"/Users/samato/Documents/workspace/syncer/LocalDir");
	private static boolean sts = true;
	private ServerConnection conn;

	/**
	 * @param args
	 */
	public static void main(String[] args) {

		ServerSync srvsync = new ServerSync();

		while (true) {
			if (sts) {
				srvsync.setConn(new ServerConnection(port, srvsync.getDir()));
				break;
			}
			try {
				Thread.sleep(1000);
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}

		try {
			srvsync.getConn().join();
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}

	/**
	 * 
	 */
	public ServerSync() {
		setup();

	}

	/**
	 * 
	 */
	void setup() {
		boolean isVis = true;

		this.setSize(300, 300);
		this.setTitle("ServerSyncer");
		this.setLocationRelativeTo(null);
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		JPanel p1 = new JPanel(new FlowLayout(FlowLayout.LEFT, 2, 2));
		p1.add(new JLabel("Server port"));
		final JTextField textfld = new JTextField("2525");
		p1.add(textfld);
		p1.setBorder(new TitledBorder("Server settings"));

		JPanel p2 = new JPanel(new FlowLayout(FlowLayout.LEFT, 2, 2));
		p2.add(new JLabel("Folder To Sync"));
		JButton browse = new JButton("browse");
		p2.add(browse);
		p2.setBorder(new TitledBorder("Folder settings"));

		this.setLayout(new GridLayout(3, 2, 2, 2));
		this.add(p1);
		this.add(p2);
		JButton done = new JButton("Done");
		this.add(done);
		this.setVisible(isVis);

		browse.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				setDir(FileUtils.selectDir());
			}
		});

		done.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub

				port = Integer.valueOf(textfld.getText());

				if (port != 0 && getDir() != null) {
					System.out.println("The port is " + port);
					System.out.println("The Directory is "
							+ getDir().getAbsolutePath());
					close();
				} else {
					System.out
							.println("Make sure to add a port and a Directory");
				}

				return;
			}
		});

	}

	/**
	 * 
	 */
	public void close() {
		this.dispose();
		sts = true;
	}

	/**
	 * @return
	 */
	public ServerConnection getConn() {
		return conn;
	}

	/**
	 * @param _conn
	 */
	public void setConn(ServerConnection _conn) {
		conn = _conn;
	}

	/**
	 * @return
	 */
	public File getDir() {
		return Dir;
	}

	/**
	 * @param dir
	 */
	public void setDir(File dir) {
		Dir = dir;
	}
}
